...
\cw{As discussed before imprecise tuning of the loop phases $\phi_i=\pi/2 +\Delta \phi_i$ of the ($\alpha_i,\beta_i$)-loop will lead to small but finite terms $\frac{\lambda^2}{E_c} \Delta \phi_i \hat z \otimes \hat z$ with random and most likely uncorrelated $\Delta \phi_i$. Thus, depending on the sign of $\Delta \phi_i$, the even (odd) parity of the two qubits is favored corresponding to a bias towards the physical $\ket{0}$ ($\ket{1}$) state, i.e., we encounter a spatially random magnetic field in the $z$-direction. The stability of the double semion model against homogeneous magnetic fields has been studied  in the literature \cite{Morampudi14} and it has been found that the topological phase persists up to values of the field strength $h_c$ up to one order of magnitude smaller than the many body Hamiltonian gap. A study invoking the duality of $\mathbb Z_2$ topological ordered Hamiltonians and the random bond Ising model substantiates the intuition that trading homogenous magnetic fields for disordered (bipartite randomness) only increases the range of stability \cite{Tsomokos2011}. Even though the model of randomness is slightly different than the one we encounter, it is reasonable to assume that a phase transition is encountered only if the average absolute value of $\Delta \phi_i$ exceeds the critical value for homogeneous magnetic fields $h_c \approx 0.1 \Delta$, where $\Delta$ is the many body gap of the double semion Hamiltonian with infinite strength of the vertex operator and thus $\Delta$ proportional to the strength of the plaquette operator. In our case, the double semion model arises in 6th order perturbation theory of the perturbative PEPS parent Hamiltonian and thus the gap scales as $\epsilon^6/\Delta_v^5$ where $\Delta_v \sim \lambda^6/E_c^5$ is the energy scale of $H_v$ and $\epsilon \sim \lambda_\text{Bell}^2/E_c$ is the energy scale of the Bell pair Hamiltonian. Assuming a separation of energy scales $\epsilon \ll \Delta_v$ and $\lambda \ll E_c$ by one order of magnitude each (random assumption..) we obtain that the critical phase variance is of the order of $10^{-10}$. However, studies of effective qubit Hamiltonians from Majorana networks based on duality transformations transformations indicate that the results of perturbation theory extend to regimes where the energy scales are much less seperated \cite{PhysRevLett.108.260504} and thus it is likely that the critical phase variance is several orders of magnitudes higher than the estimate provided here.
\tt The by far more interesting point is what happens if we do not assume that we can first calculate the effective Hamiltonian and than consider the magnetic field as a perturbation to the DS Hamiltonian, but if we treat the Bell pairs and the magnetic field together as a perturbation to $H_v$ right from the beginning. This can not be done using self-energy approximations (or at least it feels very unphysical, because we obtain field strengths that scale extensively (i.e. with the number of sites)). It seems much more sensible to use local SW transformation avoiding non-local terms and extensive scaling alltogether. This should be done to justify the whole gadget approach. It's great that the effective Hamiltonian has LTQO and is thus stable, but technically this does not apply on the level of the gadget Hamiltonian. To work this out will take at least several weeks and is not sth to be published in this paper.}
